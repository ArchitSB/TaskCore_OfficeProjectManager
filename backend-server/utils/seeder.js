...
const FIRST_NAMES = ['Aarav', 'Mira', 'Rohan', 'Isha', 'Kunal', 'Neha', 'Arjun', 'Tara', 'Kabir', 'Siya'];
const LAST_NAMES = ['Sharma', 'Patel', 'Mehta', 'Verma', 'Rao', 'Nair', 'Kapoor', 'Singh', 'Iyer', 'Joshi'];
...
const buildDueDate = () => {
  const offsetDays = randomInt(-10, 20);
  const date = new Date();
...
const seedDatabase = async () => {
  ...
  const randomMembers = buildRandomMembers(5);
  ...
  const projectsPayload = PROJECT_TEMPLATES.slice(0, projectCount).map((project) => ({
    ...
    createdBy: admin._id,
    members: pickUniqueMembers(members, randomInt(2, 3)),
  }));

  const projects = await Project.insertMany(projectsPayload);

  const taskCount = randomInt(20, 30);
  const tasksPayload = [];
